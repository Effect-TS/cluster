        ...
        if (Option.isSome(test) && Option.isSome(test.value[0])) {
          ...
          return Effect.flatMap(sharding.isShuttingDown, isGoingDown => {
            if (isGoingDown) {
              ...
              return Effect.map(_ => [_.someQueue, HashMap.set(map, entityId, [_.someQueue, _.expirationFiber])])(Effect.let("someQueue", _ => Option.some(_.queue))(Effect.tap(_ => Effect.forkDaemon(Effect.ensuring(Effect.zipRight(Fiber.interrupt(_.expirationFiber))(Effect.zipRight(Queue.shutdown(_.queue))(RefSynchronized.update(entities, HashMap.remove(entityId)))))(behavior(entityId, _.queue))))(Effect.bind("expirationFiber", () => startExpirationFiber(entityId))(Effect.bind("queue", () => Queue.unbounded())(Effect.Do)))));
            }
          });
        }
      }
