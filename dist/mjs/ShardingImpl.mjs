...
isShuttingDownRef, shardManager, pods, storage, serialization, eventsHub) {
  ...
  function registerEntity(entityType, behavior, poisonPill, entityMaxIdleTime = Option.none()) {
    return Effect.asUnit(Effect.zipRight(Hub.publish(eventsHub, ShardingRegistrationEvent.EntityRegistered(entityType)))(registerRecipient(entityType, behavior, poisonPill, entityMaxIdleTime)));
  }
  function registerTopic(topicType, behavior, poisonPill) {
    return Effect.asUnit(Effect.zipRight(Hub.publish(eventsHub, ShardingRegistrationEvent.TopicRegistered(topicType)))(registerRecipient(topicType, behavior, poisonPill, Option.none())));
  }
  const getShardingRegistrationEvents = Stream.fromHub(eventsHub);
  function registerRecipient(recipientType, behavior, poisonPill, entityMaxIdleTime = Option.none()) {
    return Effect.gen(function* ($) {
      const entityManager = yield* $(EntityManager.make(recipientType, behavior, poisonPill, self, config, entityMaxIdleTime));
...
)*/)( /*#__PURE__*/Effect.bind("entityStates", () => Ref.make(HashMap.empty()))( /*#__PURE__*/Effect.bind("shardsCache", () => Ref.make(HashMap.empty()))( /*#__PURE__*/Effect.bind("serialization", () => Serialization.Serialization)( /*#__PURE__*/Effect.bind("storage", () => Storage.Storage)( /*#__PURE__*/Effect.bind("shardManager", () => ShardManagerClient)( /*#__PURE__*/Effect.bind("pods", () => Pods)( /*#__PURE__*/Effect.bind("config", () => ShardingConfig.ShardingConfig)(Effect.Do)))))))))))))));
