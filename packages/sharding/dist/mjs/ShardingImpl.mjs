...
isShuttingDownRef, shardManager, pods, storage, serialization, eventsHub) {
  function decodeRequest(binaryMessage) {
    return pipe(Ref.get(entityStates), Effect.map(HashMap.get(binaryMessage.entityType)), Effect.flatMap(_ => Effect.unified(Option.match(_, {
      onNone: () => Effect.fail(ShardingError.ShardingErrorEntityTypeNotRegistered(binaryMessage.entityType, address)),
      onSome: entityState => pipe(serialization.decode(binaryMessage.body, entityState.entityManager.recipientType.schema), Effect.map(request => [request, entityState.entityManager]))
    }))));
  }
  function encodeReply(request, reply) {
    ...
    return pipe(serialization.encode(reply, request.replier.schema));
  }
  function decodeReply(request, body) {
    if (!Message.isMessage(request) && !StreamMessage.isStreamMessage(request)) {
      return Effect.die(NotAMessageWithReplierDefect(request));
    }
    return pipe(serialization.decode(body, request.replier.schema));
  }
  ...
  const unregister = pipe(shardManager.getAssignments, Effect.matchCauseEffect({
    ...
    onSuccess: () => pipe(Effect.logDebug(`Stopping local entities`), Effect.zipRight(pipe(isShuttingDownRef, Ref.set(true))), Effect.zipRight(pipe(Ref.get(entityStates), Effect.flatMap(Effect.forEach(([name, entityState]) => pipe(entityState.entityManager.terminateAllEntities, Effect.catchAllCause(_ => Effect.logError("Error during stop of entity " + name, _))), {
      discard: true
  ...
  function registerSingleton(name, run) {
    return pipe(Effect.context(), Effect.flatMap(context => Synchronized.update(singletons, list => List.prepend(list, [name, Effect.provide(run, context), Option.none()]))), Effect.zipLeft(startSingletonsIfNeeded), Effect.zipRight(Hub.publish(eventsHub, ShardingRegistrationEvent.SingletonRegistered(name))));
  }
  ...
  function sendToLocalEntityStreamingReply(msg) {
    return pipe(Effect.gen(function* (_) {
      ...
      return pipe(replyChannel.output, Stream.mapEffect(reply => encodeReply(request, reply)));
    }), Stream.fromEffect, Stream.flatten());
  }
  function initReply(id, replyChannel) {
    return pipe(replyChannels, Synchronized.update(HashMap.set(id, replyChannel)), Effect.zipLeft(pipe(replyChannel.await, Effect.ensuring(Synchronized.update(replyChannels, HashMap.remove(id))), Effect.forkIn(layerScope))));
  }
  ...
  function sendToPod(recipientTypeName, entityId, msg, msgSchema, pod, replyId, replyChannel) {
    if (config.simulateRemotePods && equals(pod, address)) {
      return pipe(serialization.encode(msg, msgSchema), Effect.flatMap(bytes => pipe(decodeRequest(BinaryMessage.make(entityId, recipientTypeName, bytes, replyId)), Effect.flatMap(([request, entityManager]) => entityManager.send(entityId, request, replyId, replyChannel)))), Effect.asUnit);
      ...
      return pipe(serialization.encode(msg, msgSchema), Effect.flatMap(bytes => {
        ...
        return pipe(replyChannel.replyStream(pipe(pods.sendMessageStreaming(pod, binaryMessage), Stream.tapError(errorHandling), Stream.mapEffect(bytes => decodeReply(msg, bytes)))));
      }));
    }
  }
  function messenger(entityType, sendTimeout = Option.none()) {
    ...
    function sendMessage(entityId, msg, replyId) {
      return pipe(sendMessageStreaming(entityId, msg, replyId), Effect.map(Stream.runHead), Effect.flatten);
    }
  ...
  function registerEntity(entityType, behavior, options) {
    return pipe(registerRecipient(entityType, behavior, options), Effect.zipRight(Hub.publish(eventsHub, ShardingRegistrationEvent.EntityRegistered(entityType))), Effect.asUnit);
  }
  function registerTopic(topicType, behavior, options) {
    return pipe(registerRecipient(topicType, behavior, options), Effect.zipRight(Hub.publish(eventsHub, ShardingRegistrationEvent.TopicRegistered(topicType))), Effect.asUnit);
  }
