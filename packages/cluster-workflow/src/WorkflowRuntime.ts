...
function handleReplayPhase<A, E>(
    ...
    return WorkflowRuntimeState.match(wrs, {
        onCompleted: () => {
            return Effect.die(`ABSURD: The journal is broken. No events can appear after a Completed event.`)
        },
        onRunning: () => {
        ...
        onReplay: (state) => {
            ...
            if (sa.sequence !== state.expectedSequence) {
                return Effect.die(`We expected to receive an event with the sequence ${state.expectedSequence} but got ${sa.sequence} instead. Seems like journal is broken or out of order.`)
            }
            ...
            return (DurableExecutionEvent.match(sa, {
                ...
                onInterruptionRequested: _ => pipe(
                ...
                onForked: _ => {
                        ...
                        return WorkflowRuntimeMessage.match(sm, {
                            ...
                            onRequestFork: fork => {
                                if (_.persistenceId !== fork.persistenceId) {
                                    return Effect.die(`Determinism has been broken! Journal expected ${_.persistenceId} to be started, but got ${fork.persistenceId} instead!`)
                                }
                                return pipe(
                    ...
                    return pipe(
                        ...
                        Effect.flatMap(Option.match({
                            onSome: (requestedFork) => Effect.succeed(new WorkflowRuntimeState.Replay({ ...state, delayedMessages: ReadonlyArray.filter(state.delayedMessages, _ => _ !== requestedFork) })),
                            onNone: () => Effect.gen(function* ($) {
                                let delayedMessages = state.delayedMessages
                                while (true) {
                                    const message = yield* $(Queue.take(mailbox))
                                    if (yield* $(processMessageWaitingForFork(message))) return new WorkflowRuntimeState.Replay({ ...state, delayedMessages })
                ...
                onJoined: _ => {
                    ...
                    return pipe(
                        ...
                        Effect.flatMap(Option.match({
                            onSome: (message) => Effect.succeed(new WorkflowRuntimeState.Replay({ ...state, delayedMessages: ReadonlyArray.filter(state.delayedMessages, _ => _ !== message) })),
                            onNone: () => Effect.gen(function* ($) {
                                let delayedMessages = state.delayedMessages
                                while (true) {
                                    const message = yield* $(Queue.take(mailbox))
                                    if (yield* $(processMessageWaitingForJoin(message))) return new WorkflowRuntimeState.Replay({ ...state, delayedMessages })
                                    delayedMessages = delayedMessages.concat([message])
                                }
                            })
                        }))
...
function handleExecutionPhase<A, E>(
    ...
    fiber: Fiber.Fiber<A, E>,
    mailbox: Queue.Dequeue<WorkflowRuntimeMessage.WorkflowRuntimeMessage<A, E>>
): Effect.Effect<WorkflowRuntimeState.WorkflowRuntimeState<A, E>, never, Scope.Scope | DurableExecutionJournal.DurableExecutionJournal> {
    return WorkflowRuntimeState.match(wrs, {
        // we need to switch from replay to running state and then run all the delayed messages one after the other
        onReplay: state => {
            return pipe(
                DurableExecutionJournal.append(persistenceId, success, failure, DurableExecutionEvent.Attempted(0)(state.expectedSequence)),
                Effect.zipRight(
                    Effect.reduce(state.delayedMessages.concat([ms]), new WorkflowRuntimeState.Running({ attempt: state.attempt + 1, nextSequence: state.expectedSequence + 1 }) as WorkflowRuntimeState.WorkflowRuntimeState<A, E>, (state, msg) => handleExecutionPhase(persistenceId, success, failure, state, msg, fiber, mailbox)
                    )))
        },
        ...
        onRunning: state => {
            return WorkflowRuntimeMessage.match(ms, {
                ...
                onRequestComplete: message => pipe(
                    DurableExecutionJournal.append(persistenceId, success, failure, DurableExecutionEvent.Completed(message.exit)(state.nextSequence)),
                ...
                onRequestFork: message => pipe(
                    DurableExecutionJournal.append(persistenceId, success, failure, DurableExecutionEvent.Forked(message.persistenceId)(state.nextSequence)),
                ...
                onRequestJoin: message => pipe(
                    DurableExecutionJournal.append(persistenceId, success, failure, DurableExecutionEvent.Joined(message.persistenceId)(state.nextSequence)),
                    Effect.as(new WorkflowRuntimeState.Running({ ...state, nextSequence: state.nextSequence + 1 })),
                    Effect.zipLeft(Deferred.succeed(message.signal, void 0))
                ),
        ...
        onCompleted: state => Effect.never,
    })
}
...
export function attempt<A extends Message.Message.Any, R>(workflow: Workflow.Workflow<A, R>) {
    return (request: A): Effect.Effect<Message.Message.Success<A>, Message.Message.Error<A>, R | DurableExecutionJournal.DurableExecutionJournal> =>
        Effect.gen(function* ($) {
            ...
            const initialState = new WorkflowRuntimeState.Replay({ expectedSequence: 0, attempt: 0, delayedMessages: [] }) as WorkflowRuntimeState.WorkflowRuntimeState<Message.Message.Success<A>, Message.Message.Error<A>>
            ...
            const isYielding = pipe(
                Ref.get(stateRef),
                Effect.map(_ => WorkflowRuntimeState.match(_, {
                    onYielding: () => true,
                    onReplay: () => false,
                    onCompleted: () => false,
                    onRunning: () => false
            ...
            const forkAndJoin = <A2, E2, R2>(persistenceId: string, fa: Effect.Effect<A2, E2, R2>) => pipe(
                Deferred.make<void>(),
                Effect.tap(signal => Queue.offer(mailbox, new WorkflowRuntimeMessage.RequestFork({ persistenceId, signal }))),
                Effect.tap(Deferred.await),
                Effect.zipRight(fa),
                Effect.zipLeft(pipe(Deferred.make<void>(),
                    Effect.tap(signal => Queue.offer(mailbox, new WorkflowRuntimeMessage.RequestJoin({ persistenceId, signal }))),
                    Effect.tap(Deferred.await)
                )
                )
            )
            ...
            const fiber = yield* $(
                ...
                Effect.provideService(WorkflowContext.WorkflowContext, { forkAndJoin, durableExecutionJournal, makePersistenceId, isYielding, yieldExecution }),
            ...
            yield* $(
                ...
                Stream.runFoldEffect(initialState, (state, event) => pipe(
            ...
            return yield* $(
                Queue.take(mailbox),
                Effect.flatMap(message => pipe(
                    Ref.get(stateRef),
                    Effect.flatMap(state => handleExecutionPhase(persistenceId, successSchema, failureSchema, state, message, fiber, mailbox)),
                    Effect.tap(state => Ref.set(stateRef, state)),
                    Effect.uninterruptible
                )),
                Effect.forever,
                Effect.race(Fiber.await(fiber)),
                Effect.flatten
            )
        }).pipe(Effect.scoped)
}
